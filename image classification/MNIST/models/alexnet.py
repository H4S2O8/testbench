...
class AlexNet(nn.Module):
    def __init__(self, num_classes: int = 10, float=False) -> None:
        ...
        if float:
            self.layer1 = Dummy(nn.Sequential(nn.Conv2d(1, 96, 3, stride=1, padding=1), # Changed from 3 input channels to 1
                                              nn.BatchNorm2d(96),
            ...
            self.layer1 = Dummy(nn.Sequential(first_conv(1, 96, 3, stride=1, padding=1), # Changed from 3 input channels to 1
                                              nn.BatchNorm2d(96),
...
class S_AlexNet(nn.Module):
    def __init__(self, num_classes: int = 10, T: int = 3) -> None:
        ...
        self.layer1 = Spiking(nn.Sequential(first_conv(1, 96, 3, stride=1, padding=1), # Changed from 3 input channels to 1
                                            nn.BatchNorm2d(96),
                                            IF()), T)
