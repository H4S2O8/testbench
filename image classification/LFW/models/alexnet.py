...
class AlexNet(nn.Module):
    def __init__(self, num_classes: int = 5749, float=False) -> None:
        super(AlexNet, self).__init__()
        self.num_layers = 8
        self.flatten_id = 6
        if float:
            self.layer1 = Dummy(nn.Sequential(nn.Conv2d(3, 64, kernel_size=11, stride=4, padding=2),
                                                nn.BatchNorm2d(64),
                                                nn.ReLU(inplace=True)))
            self.layer2 = Dummy(nn.Sequential(nn.Conv2d(64, 192, kernel_size=5, stride=2,padding=1),
                                                nn.BatchNorm2d(192),
                                                nn.ReLU(inplace=True)))
            self.layer3 = Dummy(nn.Sequential(nn.Conv2d(192, 384, kernel_size=3, stride=2, padding=0),
                                                nn.BatchNorm2d(384),
                                                nn.ReLU(inplace=True)))
            self.layer4 = Dummy(nn.Sequential(nn.Conv2d(384, 256, kernel_size=3, padding=1),
                                                nn.BatchNorm2d(256),
                                                nn.ReLU(inplace=True)))
            self.layer5 = Dummy(nn.Sequential(nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=0),
                                                nn.BatchNorm2d(256),
                                                nn.ReLU(inplace=True)))
            self.layer6 = Dummy(nn.Sequential(nn.Linear(256 * 6 * 6, 4096),
                                                nn.BatchNorm1d(4096),
                                                nn.ReLU(inplace=True)))
            self.layer7 = Dummy(nn.Sequential(nn.Linear(4096, 4096),
                                                nn.BatchNorm1d(4096),
                                                nn.ReLU(inplace=True)))
            ...
            self.layer2 = Dummy(nn.Sequential(QuantConv2d(64, 192, kernel_size=5, stride=2,padding=1),
                                              nn.BatchNorm2d(192),
                                              QuantReLU(inplace=True)))
            self.layer3 = Dummy(nn.Sequential(QuantConv2d(192, 384, kernel_size=3, stride=2, padding=0),
                                              nn.BatchNorm2d(384),
                                              QuantReLU(inplace=True)))
            self.layer4 = Dummy(nn.Sequential(QuantConv2d(384, 256, kernel_size=3, padding=1),
                                              nn.BatchNorm2d(256),
                                              QuantReLU(inplace=True)))
            self.layer5 = Dummy(nn.Sequential(QuantConv2d(256, 256, kernel_size=3, stride=2, padding=0),
                                              nn.BatchNorm2d(256),
                                              QuantReLU(inplace=True)))
            self.layer6 = Dummy(nn.Sequential(QuantLinear(256 * 6 * 6, 4096),
                                              nn.BatchNorm1d(4096),
                                              QuantReLU(inplace=True)))
            self.layer7 = Dummy(nn.Sequential(QuantLinear(4096, 4096),
                                              nn.BatchNorm1d(4096),
                                              QuantReLU(inplace=True)))
            self.layer8 = Dummy(last_fc(4096, num_classes))
...
class S_AlexNet(nn.Module):
    def __init__(self, num_classes: int = 5749, bit: int = 32) -> None:
